...
# Calculate percentage delta values
delta_values_webp_hi = ((org_hi['plt'] - webp_hi['plt']) / org_hi['plt']) * 100
delta_values_avif_hi = ((org_hi['plt'] - avif_hi['plt']) / org_hi['plt']) * 100
delta_values_webp_li = ((org_li['plt'] - webp_li['plt']) / org_li['plt']) * 100
delta_values_avif_li = ((org_li['plt'] - avif_li['plt']) / org_li['plt']) * 100
...
ax.set_xlabel('Percentage Delta Values for Page Load Time', fontsize=12)
...
plot.savefig('cdf_%delta_plt.pdf', bbox_inches='tight')
plot.show()
